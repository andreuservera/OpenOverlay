...
internal static class DeltaBuilder
{
    ...
    {
        var (varName, okName, label) = reference switch
        {
            DeltaReference.SessionBest => (TelemetryVarNames.DeltaToSessionBestLap, TelemetryVarNames.DeltaToSessionBestLapOk, "VS SESSION BEST"),
            DeltaReference.PersonalBestAllTime => (TelemetryVarNames.DeltaToBestLap, TelemetryVarNames.DeltaToBestLapOk, "VS ALL-TIME BEST"),
            DeltaReference.OptimalLap => (TelemetryVarNames.DeltaToOptimalLap, TelemetryVarNames.DeltaToOptimalLapOk, "VS OPTIMAL LAP"),
            _ => (TelemetryVarNames.DeltaToSessionBestLap, TelemetryVarNames.DeltaToSessionBestLapOk, "VS SESSION BEST"),
        };

        if (!telemetry.HasVariable(varName))
        {
            return new DeltaState { DeltaSeconds = 0, IsValid = false, ReferenceLabel = label };
        }
