...
/// Stateful across ticks (keeps a rolling baseline of the "normal" ratio); one instance per session,
/// call from a single thread (the UI timer).
/// </summary>
internal sealed class WheelSlipDetector
{
    ...
    public bool Update(float rpm, float speedMps, float throttle, int gear)
    {
        if (gear <= 0 || speedMps < MinSpeedMps || throttle < MinThrottle || rpm <= 0)
        {
            ...
            // don't flag, and don't let a meaningless sample corrupt the baseline.
            return false;
        }

        var ratio = rpm / speedMps;

        if (_baselineRatio is not { } baseline)
        {
            _baselineRatio = ratio;
            return false;
        }

        var isSlipping = ratio > baseline * SpikeThreshold;
        ...
        if (!isSlipping)
        {
            ...
            _baselineRatio = baseline + (ratio - baseline) * BaselineSmoothing;
        }
